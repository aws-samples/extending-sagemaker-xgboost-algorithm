...
def predict(model, model_format, dtest, input_content_type, objective=None):
    ...
    if isinstance(model, list):
        logging.info('Computing shap values not supported in multi-model mode.')
        ...
        preds = model.predict(dtest, ntree_limit=getattr(model, "best_ntree_limit", 0),
                              validate_features=False)
        exp_val = explainer.expected_value
        return np.vstack((preds, np.full(preds.shape, exp_val), shap_values.transpose())).transpose()
